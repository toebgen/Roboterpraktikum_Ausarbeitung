...
Weil die explizite Berechnung des kollisionsfreien Konfigurationsraums bei Robotern mit mehr Freiheitsgraden als 3 sehr teuer ist, benutzt man oft den impliziten Konfigurationsraum.
...
Außerdem kann man den Grundalgorithmus noch sehr viel optimieren.
...
Es gibt weitere Erweiterungen für bidirektionale Suche, parallele \textit{A*}-Suche und dynamische Auswahl von Start und Zielknoten.
...
Diese Erweiterungen sind von den zu lösenden Aufgabe abhängig, mehrere Start und Zielknoten machen zum Beispiel bei mobilen Robotern nicht viel Sinn, dafür könnte es beim Schweißen nützlich sein.
...
\begin{itemize}
	\item \lstinline{StoreRoboterPositionsToFile}: Während dem Modus \lstinline{followPerson} (s. Abs. \ref{planer_integration_sec} für weitere Informationen zur globalen Zustandsmaschine) aktiv
	\item \lstinline{LoadRoboterPositionsFromFile}: Während dem Modus Während dem Modus \lstinline{followPath} (s. Abs. \ref{planer_integration_sec}) aktiv
	\item \lstinline{fileUtil}
\end{itemize}

Im globalen Modus \lstinline{followPerson} (s. Abs. \ref{planer_integration_sec}), also während dem Einlernen eines neuen Pfades, schreibt das Modul \lstinline{StoreRoboterPositionsToFile} in regelmäßigen Abständen $d_{save}$ Roboterposen in eine externe Datei (sog. \lstinline{dumpFile}).
...
Im globalen Modus \lstinline{loadPath} (s. Abs. \ref{planer_integration_sec}), also während dem Abfahren eines gespeicherten Pfades, liest das Modul \lstinline{LoadRoboterPositionsFromFile} die Punkte aus dem \lstinline{dumpFile}.
...
\begin{figure}
	\centering
	\label{fig:bahnplanung_umsetzung_algorithm_antigrav}
	\includegraphics[scale=0.9]{graphics/BILD-AlgoAstar.png}
	\caption{Darstellung der Hinderniskarte mit Antigravitationsfeld und der gefundenen Trajektorie mit Hilfe des A*-Algorithmus.}
\end{figure}

\todo[inline]{Bild ohne Antig-Feld und mit einfacher Vergrößerung}
